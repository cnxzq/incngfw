...
function probe(url, timeout) {
  return new Promise((resolve) => {
    ...
    request = transport.request(url, { method: 'HEAD' }, (response) => {
      response.resume();
...
function isGithubRawAccessible(options) {
  const normalized = normalizeProbeOptions(options);
  const key = JSON.stringify({ type: 'github-raw', ...normalized });
  const now = Date.now();
  const existing = sharedChecks.get(key);
  if (existing) {
    if (existing.promise) return existing.promise;
  ...
  const entry = {};
  const promise = probe(GITHUB_RAW_URL, normalized.timeout).then((result) => {
    const accessible = result.reachable;
    if (normalized.cacheTtl > 0) {
      entry.promise = null;
      entry.result = accessible;
      entry.expiresAt = Date.now() + normalized.cacheTtl;
      entry.timer = setTimeout(() => {
        if (sharedChecks.get(key) === entry) sharedChecks.delete(key);
      }, normalized.cacheTtl);
